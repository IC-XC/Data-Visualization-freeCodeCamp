...
d3.json(dataUrl)
	...
	.then(d => {
		...
		// Treemap background
		let diagram = d3.select('#treemap_diagram')
								.append('svg')
								.attr('width', width)
								.attr('height', height);
		...
		let genres = diagram.selectAll('g')
									.data(treemapData.children)
									.enter()
									.append("g");
									
		let movies = genres.selectAll('g')
								.data((d) => (d.children)) 
								.enter()
								.append('g')
		...
		movies.append('rect')
					...
					.on('mouseover', (d, i) => {
						...
						tooltip.html('<div style="font-size: 12px; font-weight: bold; color: red; text-align: center;">'
							+ d.data.name + '<br>' + d.data.category +  '<br>$' + d.data.value + '</div>')
